...
%% Update plot
    function updatePlot(hObject)
        ...
        fast_nullcline_piecewise = @(xs,xf,u,gamma,beta,alpha,delta,ef)...
            ((-xf + tanh(piece_wise_bump_2(u + xs +0.5*gamma*xf) + beta*xf+ alpha))/ef);
        ...
        betaOffset = getappdata(hObject.Parent,'bOffset')
        betaRelay = beta - betaOffset;
        ...
        hold on
        h2 = ezplot(@(x,y)fast_nullcline_relay_piecewise(x,y,u,gamma,betaRelay,alpha,delta,tf),[-4,4]);
        set(h1,'Color','b');
        set(h2,'Color','r');
